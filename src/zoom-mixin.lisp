...
(defmethod zoom-x-update ((self zoom-mixin) io)
  (when (and (/= .0 (plus-c:c-ref io ig:im-gui-io :mouse-wheel))
             (ig:ensure-to-bool (plus-c:c-ref io ig:im-gui-io :key-ctrl))
             (ig:ensure-to-bool (plus-c:c-ref io ig:im-gui-io :key-alt)))
    (let* ((zoom-x-new (max (+ (.zoom-x self)
                               (* (.zoom-x self)
                                  (plus-c:c-ref io ig:im-gui-io :mouse-wheel)
                                  (.zoom-x-factor self)))
                            (.zoom-x-min self)))
...
(defmethod zoom-y-update ((self zoom-mixin) io)
  (when (and (/= .0 (plus-c:c-ref io ig:im-gui-io :mouse-wheel))
             (ig:ensure-to-bool (plus-c:c-ref io ig:im-gui-io :key-ctrl))
             (not (ig:ensure-to-bool (plus-c:c-ref io ig:im-gui-io :key-alt))))
    (let* ((zoom-y-new (max (+ (.zoom-y self)
                               (* (.zoom-y self)
                                  (plus-c:c-ref io ig:im-gui-io :mouse-wheel)
                                  (.zoom-y-factor self)))
                            (.zoom-y-min self)))
