...
(defmethod initialize-instance :after ((self vst3-module) &key)
  (let ((host-applicaiton (make-instance 'vst3-impl::host-application :module self))
        (pd (autowrap:alloc '(:struct (vst3-c-api:steinberg-vst-process-data))))
        ...
        (input-parameter-changes        TODO impl))
    (vst3-impl::add-ref host-applicaiton)
    (setf (slot-value self 'host-applicaiton) host-applicaiton)

    (setf (slot-value self 'process-data) pd)
    (setf (vst3-c-api:steinberg-vst-process-data.process-mode pd)
          vst3-c-api:+steinberg-vst-process-modes-k-realtime+)
    (setf (vst3-c-api::steinberg-vst-process-data.symbolic-sample-size pd)
          vst3-c-api::+steinberg-vst-symbolic-sample-sizes-k-sample32+)
    (setf (vst3-c-api:steinberg-vst-process-data.num-samples pd)
          *frames-per-buffer*)
    (setf (vst3-c-api:steinberg-vst-process-data.num-inputs pd)
          0)
    (setf (vst3-c-api:steinberg-vst-process-data.num-outputs pd)
          1)
    (setf (vst3-c-api:steinberg-vst-process-data.inputs pd)
          inputs)
    (setf (vst3-c-api:steinberg-vst-process-data.outputs pd)
          outputs)
    (setf (vst3-c-api:steinberg-vst-process-data.input-parameter-changes pd)
          input-parameter-changes)
    (setf (vst3-c-api:steinberg-vst-process-data.output-parameter-changes pd)
          (cffi:null-pointer))
    (setf (vst3-c-api:steinberg-vst-process-data.input-events pd)
          (cffi:null-pointer))
    (setf (vst3-c-api:steinberg-vst-process-data.output-events pd)
          (cffi:null-pointer))
    (setf (vst3-c-api:steinberg-vst-process-data.process-context pd)
          (cffi:null-pointer))))
