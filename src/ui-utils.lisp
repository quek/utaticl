...
(defun shortcut-common (project)
  ...
  (defshortcut (ig:+im-gui-mod-ctrl+ ig:+im-gui-key-p+)
    (cmd-add project 'cmd-paste))
  ...
  (defshortcut (ig:+im-gui-mod-ctrl+ ig:+im-gui-key-x+)
    (cmd-add project 'cmd-cut))
...
(defmacro with-window-info ((self) &body body)
  `(progn
     (setf *window-pos* (ig:get-window-pos))
     (setf *window-size* (ig:get-window-size))
     (setf *scroll-x* (ig:get-scroll-x))
