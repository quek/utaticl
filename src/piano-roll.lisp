    ...
    (if (ig:is-mouse-released ig:+im-gui-mouse-button-left+)
        ...
        (if (.range-selecting-pos1 self)
            (multiple-value-bind (time key)
              ...
              (let ((delta-time (- time (world-x-to-time self (.x (.range-selecting-pos1 self)))))
                    (delta-key (- key (world-y-to-key self (.y (.range-selecting-pos1 self))))))
